...
**Status: the runtime compiles, and the stdlib cross-compiles. Nothing runs on a
device yet.**
...
- **Bootstrap.** Started, not finished. A host stage0 driving a cross-compiled
  stage1 via `STAGE1_PREV_STAGE` does work: 2,324 `.olean` files build and generated
  Lean code compiles to ARM aarch64 objects. Three cross-compilation gaps turned up,
  two worked around and one open (the host `libtool` archiving Android objects).
  Full write-up in [BOOTSTRAP.md](BOOTSTRAP.md).
